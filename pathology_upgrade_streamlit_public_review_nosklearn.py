...
# ---------------- Fixed summary logic for manuscript-consistent website counts ----------------
TRAIN_PATH = Path("训练集.xlsx")               # expected N = 255
VALIDATION_PATH = Path("验证集.xlsx")         # expected N = 109
EXTERNAL_PATH = Path("External_Validation_Patients.csv")  # expected N = 44

MORPH_OPTIONS = ["a", "c", "b"]
MORPH_MAP = {"a": 0, "c": 1, "b": 2}
IPCL_OPTIONS = [1, 2, 3, 4]

CLIN_MEANS = {"IPCL": 1.8681318681318682, "瘤内溃疡": 0.12454212454212454, "表面充血糜烂": 0.2454212454212454, "结节": 0.42124542124542125, "大小": 3.389364303178483, "病变形态_code": 0.8534798534798534, "SIRI": 0.8761221643312102}
CLIN_STDS = {"IPCL": 0.9295214098351234, "瘤内溃疡": 0.3301885321736297, "表面充血糜烂": 0.43033861235242114, "结节": 0.49375615490669355, "大小": 1.5307717780603904, "病变形态_code": 0.8966603961424088, "SIRI": 0.8868546218967986}
CLIN_COEF = {"const": 0.6651062805711763, "IPCL": 1.6811844946252706, "瘤内溃疡": 0.3952093844683807, "表面充血糜烂": 0.5309375665678092, "结节": 0.10002743037434834, "大小": 0.8754363954074059, "病变形态_code": -0.43043818254493084, "SIRI": 0.14665729221137015}

FUSION_MEANS = {"IPCL": 1.8681318681318682, "瘤内溃疡": 0.12454212454212454, "表面充血糜烂": 0.2454212454212454, "结节": 0.42124542124542125, "大小": 3.389364303178483, "病变形态_code": 0.8534798534798534, "SIRI": 0.8761221643312102, "CNNpred_Mean": 0.7366758687626536}
FUSION_STDS = {"IPCL": 0.9295214098351234, "瘤内溃疡": 0.3301885321736297, "表面充血糜烂": 0.43033861235242114, "结节": 0.49375615490669355, "大小": 1.5307717780603904, "病变形态_code": 0.8966603961424088, "SIRI": 0.8868546218967986, "CNNpred_Mean": 0.14950197590783872}
FUSION_COEF = {"const": 0.51926929103262, "IPCL": 1.3569965009859741, "瘤内溃疡": 0.13790272535939233, "表面充血糜烂": 0.36513800459113693, "结节": -0.09792609128090361, "大小": 0.8961129826861076, "病变形态_code": -0.6300639499778794, "SIRI": 0.2729278505820768, "CNNpred_Mean": 2.576734824602877}
...
def explain_contributions(raw_values: dict, means: dict, stds: dict, coefs: dict):
    ...
    for key in means.keys():
        ...
        c = z * coefs[key]
        rows.append({"Feature": key.replace("_code", ""), "Contribution": c, "Direction": "↑风险" if c >= 0 else "↓风险"})
...
def load_cohort_summary():
    ...
    if TRAIN_PATH.exists():
        try:
            rows.append({"Cohort": "Training", "N": len(pd.read_excel(TRAIN_PATH))})
        except Exception:
            rows.append({"Cohort": "Training", "N": "read_error"})
    else:
        rows.append({"Cohort": "Training", "N": "missing"})
    if VALIDATION_PATH.exists():
        try:
            rows.append({"Cohort": "Validation", "N": len(pd.read_excel(VALIDATION_PATH))})
        except Exception:
            rows.append({"Cohort": "Validation", "N": "read_error"})
    else:
        rows.append({"Cohort": "Validation", "N": "missing"})
    if EXTERNAL_PATH.exists():
        try:
            rows.append({"Cohort": "External validation", "N": len(pd.read_csv(EXTERNAL_PATH))})
        except Exception:
            rows.append({"Cohort": "External validation", "N": "read_error"})
    else:
        rows.append({"Cohort": "External validation", "N": "missing"})
...
def batch_predict(df_input: pd.DataFrame):
    ...
    for _, row in df_input.iterrows():
        ...
        result_rows.append({**row.to_dict(), "Clinical_LR_like": cp, "Fusion_LR_like": fp, "Risk_Label@0.50": "病理升级高风险" if fp >= 0.5 else "病理升级低风险"})
    return pd.DataFrame(result_rows)

st.markdown('<div class="main-title">病理升级在线预测平台（审稿人公开版）</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">本版网站的显示口径已固定为 Training / Validation / External = 255 / 109 / 44，以与文章保持一致。</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    st.caption("该在线版不重训模型，只调用锁定系数；右侧 cohort 数字固定来自 训练集.xlsx / 验证集.xlsx / External_Validation_Patients.csv。")
...
with tab1:
    ...
    with left:
        ...
        if submitted:
            raw = {"IPCL": float(ipcl), "瘤内溃疡": float(ulcer), "表面充血糜烂": float(hyperemia), "结节": float(nodule), "大小": float(size), "病变形态_code": float(MORPH_MAP.get(morph, 0)), "SIRI": float(siri), "CNNpred_Mean": float(cnnprob)}
            ...
            st.dataframe(explain_contributions(raw, FUSION_MEANS, FUSION_STDS, FUSION_COEF).round(4), use_container_width=True, hide_index=True)
...
with tab2:
    ...
    sample_df = pd.DataFrame([{"姓名": "示例患者", "IPCL": 2, "瘤内溃疡": 0, "表面充血糜烂": 1, "结节": 0, "大小": 3.5, "病变形态": "a", "SIRI": 1.23, "CNNpred_Mean": 0.67}])
    buf = io.BytesIO()
    ...
    if uploaded is not None:
        ...
        if missing_cols:
            ...
            st.download_button("下载预测结果 Excel", data=out_buf.getvalue(), file_name="batch_prediction_results.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

with tab3:
    st.subheader("说明")
    st.markdown("""
    **本次更新：**
    - 右侧 cohort 统计已按文章要求固定为 Training / Validation / External = 255 / 109 / 44  
    - Training 直接读取 `训练集.xlsx`  
    - Validation 直接读取 `验证集.xlsx`  
    - External 直接读取 `External_Validation_Patients.csv`  
